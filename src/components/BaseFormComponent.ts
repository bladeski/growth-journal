...
> extends BaseComponent<TProps, TEvents> {
  ...
  protected updateMessages(): void {
    ...
    const errorMsg = this.shadowRoot.querySelector('#error-message') as MessageComponent;
    const successMsg = this.shadowRoot.querySelector('#success-message') as MessageComponent;

    if (errorMsg) {
      errorMsg.updateMessage(this.props.errorMessage, 'error');
    }
    if (successMsg) {
      successMsg.updateMessage(this.props.successMessage, 'success');
    }
  }
 ...
 */
export interface FormProps extends PropTypes {
  errorMessage: string;
  successMessage: string;
  isLoading: boolean;
